...
const RequestAsset = () => {
    ...
    const [requests, setRequests] = useState([]); // user's requests

    // Fetch available assets and user's existing requests
    useEffect(() => {
    ...
    const handleRequest = async () => {
        ...
        try {
            ...
            setRequests(prev => {
                // Remove any old rejected requests for this asset
                const filtered = prev.filter(r => r.assetId !== selectedAsset._id);
                return [...filtered, requestData];
            });
    ...
    if (loading) return <div className="text-center py-10">Loading assets...</div>;
        ...
        <div className="p-6">
            ...
            {assets.length === 0 ? (
                <div className="text-center py-10">
                    ...
                    <p className="text-gray-500">Currently there are no assets available to request.</p>
                </div>
            ) : (
                <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    {assets.map((asset) => {
                        ...
                        // Show request button if no request exists OR previous request was rejected
                        const showRequestButton = !userRequest || userRequest.requestStatus === "rejected";
            ...
            <dialog id="request_modal" className="modal">
                <form method="dialog" className="modal-box bg-white rounded-2xl p-6 shadow-lg flex flex-col gap-4">
